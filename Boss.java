 ...
 */
public class Boss extends Inimigo_Composit {
	private int atira;
	private Image image;
	...
	public void start() {
		...
		elements.add(new Element_Boss(getX(), getY()));
		elements.add(new Element_Boss(getX() + 40, getY()));
		elements.add(new Element_Boss(getX() + 80, getY()));
		
		elements.add(new Element_Boss(getX(), getY() + 40));
		elements.add(new Element_Boss(getX() + 40, getY() + 40));
		elements.add(new Element_Boss(getX() + 80, getY() + 40));
	}
	...
	public void testaColisao(Character outro){
		...
		for(int i=0;i<elements.size();i++){
			...
			if (elements.get(i).jaColidiu()){
				...
				if (elements.size() == 0){
					Params.CONT_RANK++;
					Params.VIEW_RANK.setText("Pontuação_Rank: " + Params.CONT_RANK);
					deactivate();
				}
			}
		}
